	...
	################MODS########
		...
		def obtenerConfiguracion(self):
			...
			try:
				...
				sucursal=1
				tipo=0
				infile = open(rutaUsuariov+"eum.conf", "w")
		...
		def panelConfig(self):
			global plaza,localidad,equipo,host,ip
		...
		def pollingConexion(self):
			...
			try:
				...
				#conexion_activa = conexion.pollConexion(0,str(2)+str(2))
				#conexion_activa = conexion.pollConexion(0,str(2)+str(1))
				#conexion_activa = conexion.pollConexion(0,str(1)+str(1))
				#conexion_activa = conexion.pollConexion(0,str(1)+str(2))
		...
		def montos(self):
			...
			if(pagado==2 or CobroFinalizado==1):
				pagado=0
				CobroFinalizado = 0
				print("Cajero Suspendido:",cajeroSuspendido)
				if(cajeroSuspendido==1):
					#self.stackedWidget.setCurrentIndex(14)
		...
		if registraPago:
			if not cambio_faltante :
				...
				if not cajeroSuspendido:
					...
					print("CODIGO_ERROR: Operacion registrada, cambio incompleto",CODIGO_ERROR)
				else:
					CODIGO_ERROR = OP_CAMBIO_INCOMPLETO_SUSPENDIDO
					print("CODIGO_ERROR: Operacion registrada, cambio incompleto, cajero suspendido",CODIGO_ERROR)
