...
- RECIVE Y ACOMODA BILLETES

- ENTREGA CAMBIO


Detenido y Esperando VEND VALID/PAY VALID
...
def aceptarBilletes(estadoConexion):
...
def aceptarBilletes2(estadoConexion):
	global cp,cambio,tarifa,aux_cambio,aux,rep,ser,aux_tarifa,total,bill,killbill
	print("Otra y otra")
	while(1):
		#print("killbill",killbill)
...
def leerEscribirEntradasDigitales():
	global cp
...
def operacionesControl():
	...
	SECUENCIA_CAMBIO = 8
	ESTADO_ACEPTACION = 0
	TON_01 = Temporizador("TON_01",.001)
	while(True):
		...
		if TON_01.salida:
			...
			if contador == 1 and SECUENCIA_COBRO == 0:
				...
				if len(data) > 4:
					...
					if data[2] == 16 and ESTADO_ACEPTACION == 0: # Esperando ESCROW (Deteccion de Billete) 16/19
						ESTADO_ACEPTACION = 1
					...
					#	ESTADO_ACEPTACION = 2
						...
						ESTADO_ACEPTACION = 3
					elif data[2] == 16 and ESTADO_ACEPTACION == 3: # Esperando VEND VALID  16/21
						ESTADO_ACEPTACION = 4
						print('VEND VALID')
			...
			if contador == 2 and SECUENCIA_COBRO == 1:
				...
				elif data[2] == 16 and ESTADO_ACEPTACION == 3: # Esperando PAY VALID  21
					ESTADO_ACEPTACION = 4
					r = comunicacionJcm(ACK_JCM)
					print('ACK',r)


				elif data[2] == 22 and ESTADO_ACEPTACION == 4: # Esperando DISABLE(INHIBIT)  22
					ESTADO_ACEPTACION = 0
			...
			if contador == 5:
				#pass
				leerEntradasAnalogicas()
				leerEscribirEntradasDigitales()
			if contador == 6:
				pass
		...
		if(cp==1):
			...
			if(killbill==1):
				...
				if(resultadoConfiguracionBilletero == b'\x00'):
					...
					aceptarBilletes(estadoConexion)
...
def monitorearChanger():
	...
	a = comunicacion.crearInstruccion(Comunicacion.PROCESO, Comunicacion.MDB_DATOS, [15, 1, 5, 0, int(ckInt), 0])
	ser.write(a);
	time.sleep(.01)
...
def resetJcm():
	############### RESETJcm POLL  #################
	resetJcm = [252, 5, 64]
	resetJcmCRC = CRC(resetJcm)
	resetJcm.append(resetJcmCRC.crc_l())
	resetJcm.append(resetJcmCRC.crc_h())
	print(resetJcm)
	lenresetJcm = len(resetJcm)+1
	#a = comunicacion.crearInstruccion(Comunicacion.PROCESO, Comunicacion.CCTALK_DATOS, resetJcm)
	#ser.write(a);
	ser.write(resetJcm)
	time.sleep(.1)
...
def statusRequest():
	...
	statusRequest = [252, 5, 17]
	statusRequestCRC = CRC(statusRequest)
	statusRequest.append(statusRequestCRC.crc_l())
	statusRequest.append(statusRequestCRC.crc_h())
	print(statusRequest)
	lenstatusRequest = len(statusRequest)+1
	#a = comunicacion.crearInstruccion(Comunicacion.PROCESO, Comunicacion.CCTALK_DATOS, statusRequest)
	#ser.write(a);
	ser.write(statusRequest)
	time.sleep(.1)
	r = ser.read(20) #Verificar en el simulador se ven 19
	trama = ''
	for data in r:
		trama = trama + hex(data)+ ' '
	print(trama)
	print("statusRequest ",r)
	#exit(0)
	#r = r[lenstatusRequest:]
	
	#time.sleep(.01)
	return trama

def enableJCM():
	enableJCM = [252, 7, 192, 255, 0]
	enableJCMCRC = CRC(enableJCM)
	enableJCM.append(enableJCMCRC.crc_l())
	enableJCM.append(enableJCMCRC.crc_h())
	print(enableJCM)
	lenenableJCM = len(enableJCM)+1
	#a = comunicacion.crearInstruccion(Comunicacion.PROCESO, Comunicacion.CCTALK_DATOS, enableJCM)
	#ser.write(a);
	ser.write(enableJCM)
	time.sleep(.1)
	r = ser.read(20) #Verificar en el simulador se ven 19
...
def securityJCM():
	securityJCM = [252, 7, 193, 255, 0]
	securityJCMCRC = CRC(securityJCM)
	securityJCM.append(securityJCMCRC.crc_l())
	securityJCM.append(securityJCMCRC.crc_h())
	print(securityJCM)
	lensecurityJCM = len(securityJCM)+1
	#a = comunicacion.crearInstruccion(Comunicacion.PROCESO, Comunicacion.CCTALK_DATOS, securityJCM)
	#ser.write(a);
	ser.write(securityJCM)
	time.sleep(.1)
	r = ser.read(20) #Verificar en el simulador se ven 19
	trama = ''
	for data in r:
...
def comunnicationModeJcm():
	comunnicationModeJcm = [252, 6, 194, 0]
	comunnicationModeJcmCRC = CRC(comunnicationModeJcm)
	comunnicationModeJcm.append(comunnicationModeJcmCRC.crc_l())
	comunnicationModeJcm.append(comunnicationModeJcmCRC.crc_h())
	print(comunnicationModeJcm)
	lencomunnicationModeJcm = len(comunnicationModeJcm)+1
	#a = comunicacion.crearInstruccion(Comunicacion.PROCESO, Comunicacion.CCTALK_DATOS, comunnicationModeJcm)
	#ser.write(a);
	ser.write(comunnicationModeJcm)
	time.sleep(.1)
	r = ser.read(20) #Verificar en el simulador se ven 19
	trama = ''
	for data in r:
		trama = trama + hex(data)+ ' '
	print(trama)
	print("comunnicationModeJcm ",r)
	#exit(0)
	#r = r[lencomunnicationModeJcm:]
	
	time.sleep(.01)
	return trama

def optionalFunctionJCM():
	optionalFunctionJCM = [252, 7, 197, 255, 0]
	optionalFunctionJCMCRC = CRC(optionalFunctionJCM)
	optionalFunctionJCM.append(optionalFunctionJCMCRC.crc_l())
	optionalFunctionJCM.append(optionalFunctionJCMCRC.crc_h())
	print(optionalFunctionJCM)
	lenoptionalFunctionJCM = len(optionalFunctionJCM)+1
	#a = comunicacion.crearInstruccion(Comunicacion.PROCESO, Comunicacion.CCTALK_DATOS, optionalFunctionJCM)
	#ser.write(a);
	ser.write(optionalFunctionJCM)
	time.sleep(.1)
	r = ser.read(20) #Verificar en el simulador se ven 19
	trama = ''
	for data in r:
		trama = trama + hex(data)+ ' '
	print(trama)
	print("optionalFunctionJCM ",r)
	#exit(0)
	#r = r[lenoptionalFunctionJCM:]
	
	time.sleep(.01)
	return trama

def inhibitJCM():
	inhibitJCM = [252, 6, 195, 0]
	inhibitJCMCRC = CRC(inhibitJCM)
	inhibitJCM.append(inhibitJCMCRC.crc_l())
	inhibitJCM.append(inhibitJCMCRC.crc_h())
	print(inhibitJCM)
	leninhibitJCM = len(inhibitJCM)+1
	#a = comunicacion.crearInstruccion(Comunicacion.PROCESO, Comunicacion.CCTALK_DATOS, inhibitJCM)
	#ser.write(a);
	ser.write(inhibitJCM)
	time.sleep(.1)
	r = ser.read(20) #Verificar en el simulador se ven 19
	trama = ''
	for data in r:
...
def ackJCM():
	ackJcm = [252, 5, 80]
	ackJcmCRC = CRC(ackJcm)
	ackJcm.append(ackJcmCRC.crc_l())
	ackJcm.append(ackJcmCRC.crc_h())
	print(ackJcm)
	lenackJcm = len(ackJcm)+1
	#a = comunicacion.crearInstruccion(Comunicacion.PROCESO, Comunicacion.CCTALK_DATOS, ackJcm)
	#ser.write(a);
	ser.write(ackJcm)
	time.sleep(.1)
	r = ser.read(20) #Verificar en el simulador se ven 19
...
def comunicacionJcm(comandoJcm):
	...
	print("comandoJcm ",comandoJcm, "Res:")
	print(trama)
	print('')
	#exit(0)
	#r = r[lencomando:]
...
def habilitarJCM():
	...
	if(estado == 1):
		limpiar()
		softwareVersion = [252,7,240,32,147]
		softwareVersionCRC = CRC(softwareVersion)
		softwareVersion.append(softwareVersionCRC.crc_l())
		softwareVersion.append(softwareVersionCRC.crc_h())
		print(softwareVersion)
		lensoftwareVersion = len(softwareVersion)+1
		#a = comunicacion.crearInstruccion(Comunicacion.PROCESO, Comunicacion.CCTALK_DATOS, softwareVersion)
		#ser.write(a);
		ser.write(softwareVersion)
		time.sleep(.1)
		r = ser.read(100) #Verificar en el simulador se ven 19
		#print("softwareVersion ",r)
		trama = ''
		for data in r:
			trama = trama + hex(data) + ' '
		print(trama)
		#exit(0)
		#r = r[lensoftwareVersion:]
...
if __name__ == "__main__":
	...
	ENABLE_SETTING_JCM = [252, 7, 192, 0, 0]
	...
	PAY_OUT_JCM = [252, 9, 240, 32, 74, 1, 1] # 1 Bill to dispense
