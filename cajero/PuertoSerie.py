...
class PuertoSerie (threading.Thread):
    ...
    def write (self, mensaje):
        try:
            ...
            print("\nError al escribir el puerto %s, debido posiblemente a una desconexion del cable de comunicacion" %self.__puertoSerie.port)
            self.puertoAbierto = False 
            return False
    ...
    def configurarDispositivo (self, *args, **kwargs):
        ...
        for key, value in kwargs.items():
            ...
            if key == "dispositivo":
                print("aqui andamos",key,value)
                puerto = obtenerNombreDelPuerto(**kwargs)
    ...
    def __str__ (self):
        a = ("Puerto %s a %d %s %s %s %d" %(self.__puertoSerie.port, self.__puertoSerie.baudrate, self.__puertoSerie.parity, self.__puertoSerie.stopbits, self.__puertoSerie.bytesize, self.__puertoSerie.timeout))
